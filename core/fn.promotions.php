...
function fn_promotion_apply_cart_rule($bonus, &$cart, &$auth, &$cart_products)
{
	...
	if ($bonus['bonus'] == 'order_discount') {
		...
		foreach ($bonus['value'] as $p_data) {
			...
			if (PRODUCT_TYPE != 'ULTIMATE' || (PRODUCT_TYPE == 'ULTIMATE' && defined('COMPANY_ID') && (fn_check_company_id('products', 'product_id', $p_data['product_id'], COMPANY_ID) || fn_ult_is_shared_product($p_data['product_id'], COMPANY_ID) == 'Y'))) {
