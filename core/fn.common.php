...
function fn_this_day_begin()
{
	...
	return gmmktime(0, 0, 0, $_date_month, $_date_day, $_date_year);
}
...
function fn_create_periods($params)
{
	...
	// Current dates
	if ($period == 'D') {
		$time_from = gmmktime(0, 0, 0, $today['mon'], $today['mday'], $today['year']);
		$time_to = TIME;

	} elseif ($period == 'W') {
		$wday = empty($today['wday']) ? "6" : (($today['wday'] == 1) ? "0" : $today['wday'] - 1);
		$wstart = getdate(strtotime("-$wday day"));
		$time_from = gmmktime(0, 0, 0, $wstart['mon'], $wstart['mday'], $wstart['year']);
		$time_to = TIME;

	} elseif ($period == 'M') {
		$time_from = gmmktime(0, 0, 0, $today['mon'], 1, $today['year']);
		$time_to = TIME;

	} elseif ($period == 'Y') {
		$time_from = gmmktime(0, 0, 0, 1, 1, $today['year']);
		$time_to = TIME;
	...
	// Last dates
		...
		$time_from = gmmktime(0, 0, 0, $today['mon'], $today['mday'], $today['year']);
		$time_to = gmmktime(23, 59, 59, $today['mon'], $today['mday'], $today['year']);

	} elseif ($period == 'LW') {
		$today = getdate(strtotime("-1 week"));
		$wday = empty($today['wday']) ? 6 : (($today['wday'] == 1) ? 0 : $today['wday'] - 1);
		$wstart = getdate(strtotime("-$wday day", gmmktime(0, 0, 0, $today['mon'], $today['mday'], $today['year'])));
		$time_from = gmmktime(0, 0, 0, $wstart['mon'], $wstart['mday'], $wstart['year']);

		$wend = getdate(strtotime("+6 day", $time_from));
		$time_to = gmmktime(23, 59, 59, $wend['mon'], $wend['mday'], $wend['year']);

	} elseif ($period == 'LM') {
		$today = getdate(strtotime("-1 month"));
		$time_from = gmmktime(0, 0, 0, $today['mon'], 1, $today['year']);
		$time_to = gmmktime(23, 59, 59, $today['mon'], date('t', strtotime("-1 month")), $today['year']);

	} elseif ($period == 'LY') {
		$today = getdate(strtotime("-1 year"));
		$time_from = gmmktime(0, 0, 0, 1, 1, $today['year']);
		$time_to = gmmktime(23, 59, 59, 12, 31, $today['year']);

	// Last dates
	} elseif ($period == 'HH') {
		$today = getdate(strtotime("-23 hours"));
		$time_from = gmmktime($today['hours'], $today['minutes'], $today['seconds'], $today['mon'], $today['mday'], $today['year']);
		$time_to = TIME;
	...
	if (!empty($timestamp)) {
		...
		if (count($ts) == 3) {
			...
			if (Registry::get('settings.Appearance.calendar_date_format') == 'month_first') {
				...
				$timestamp = gmmktime($h, $m, $s, $ts[1], $ts[0], $ts[2]);
			}
		} else {
			$timestamp = TIME;
		}
...
function fn_date_format($timestamp, $format = '%b %e, %Y')
{
	...
	$ir = ($pm) ? ($hr - 12) : $hr;
	$dy = $date['yday'];
	$fd = getdate(gmmktime(0, 0, 0, 1, 1, $y)); // first day of year
	$wn = (int) (($dy + $fd['wday']) / 7);
	if ($ir == 0) {
		$ir = 12;
	}
