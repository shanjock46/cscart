	...
	foreach ($products as &$_product) {
		...
		if (!empty($product['qty_step'])) {
			...
			for ($i = 1; $per_item <= ($amount - $product['qty_step']); $i++) {
				...
				if ((!empty($product['list_qty_count']) && ($i > $product['list_qty_count'])) || (empty($product['list_qty_count']) && ($i > 100))) {
					break;
				}
...
function fn_get_category_data($category_id = 0, $lang_code = CART_LANGUAGE, $field_list = '', $get_main_pair = true, $skip_company_condition = false, $preview = false)
{
	...
	if (PRODUCT_TYPE == 'ULTIMATE' && !$skip_company_condition) {
...
function fn_update_product($product_data, $product_id = 0, $lang_code = CART_LANGUAGE)
{
	...
	if (PRODUCT_TYPE == 'ULTIMATE') {
		...
		if (defined('COMPANY_ID')) {
			$product_data['company_id'] = !empty($product_company_id) ? $product_company_id : COMPANY_ID;
		}
...
function fn_delete_product_file($file_id)
{
	...
	db_query("DELETE FROM ?:product_files WHERE file_id = ?i", $file_id['file_id']);
	db_query("DELETE FROM ?:product_file_descriptions WHERE file_id = ?i", $file_id['file_id']);
		...
		if ($params['variants'] == true) {
			foreach ($data as $k => $v) {
				if (in_array($v['feature_type'], array('S', 'M', 'N', 'E'))) {
					...
					list($f_variants, $v_total) = fn_get_product_feature_variants($v['feature_id'], $params['product_id'], $v['feature_type'], true, $lang_code, $variants_per_page);
