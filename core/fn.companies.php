...
function fn_update_company($company_data, $company_id = 0, $lang_code = CART_LANGUAGE)
{
	...
	if (PRODUCT_TYPE == 'ULTIMATE' && !empty($company_data['storefront'])) {
		...
		if (!empty($http_exist) || !empty($https_exist)) {
			if (empty($company_id)) {
				...
				if (!empty($http_exist)) {
					fn_set_notification('E', fn_get_lang_var('error'), fn_get_lang_var('storefront_url_already_exists'));
				} else {
					fn_set_notification('E', fn_get_lang_var('error'), fn_get_lang_var('secure_storefront_url_already_exists'));
				}
