...
function fn_update_user($user_id, $user_data, &$auth, $ship_to_another, $notify_user, $send_password = false)
{
	...
	if (PRODUCT_TYPE == 'ULTIMATE') {
		if (AREA == 'A' && $user_data['user_type'] == 'C' && (empty($user_data['company_id']) ||  (defined('COMPANY_ID') &&  $user_data['company_id'] != COMPANY_ID))) {
	...
	if (!empty($user_id)) {
		...
		if (PRODUCT_TYPE == 'ULTIMATE') {
			if (AREA != 'A') {
				//we should set company_id for customer area, in administration area company_id received from form
				if ($current_user_data['user_type'] == 'A' || Registry::get('settings.Stores.share_users') == 'Y') {
					...
					$user_data['company_id'] = defined('COMPANY_ID') ? COMPANY_ID : 1;
				}
			} else {
		...
		if (PRODUCT_TYPE == 'ULTIMATE') {
			if (!empty($user_data['company_id']) || defined('COMPANY_ID') || AREA == 'A') {
				//company_id can be received when we create user account from admin area
				$company_id = !empty($user_data['company_id']) ? $user_data['company_id'] : (defined('COMPANY_ID') ? COMPANY_ID : 1);
			...
			if (AREA == 'A' && $user_data['user_type'] == 'A') {
				$user_data['company_id'] = $current_user_data['company_id'] = 0;
			}
		}
	...
	if (PRODUCT_TYPE == 'ULTIMATE') {
		$company = fn_get_company_data($user_data['company_id']);

		$user_data['company_name'] = $company['company'];
		$from_email['email'] = CSettings::instance()->get_value('company_users_department', 'Company', $company['company_id']);
		$from_email['name'] = $company['company'];
		if (!empty($company['storefront'])) {
			$user_data['http_location'] = 'http://' . $company['storefront'] . '/' . fn_get_index_script($user_data['user_type']);
		}
	}
