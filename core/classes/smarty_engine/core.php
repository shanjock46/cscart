...
class SmartyEngine_Core extends Smarty
{
	...
	function display_mail($tpl, $to_screen, $company_id = null, $lang_code = CART_LANGUAGE)
	{
		...
		if (PRODUCT_TYPE == 'ULTIMATE' && !empty($company_id)) {
			// execute display for selected company
			$this->init_dirs($company_id);
		}
		...
		if (PRODUCT_TYPE == 'ULTIMATE' && $company_id != null) {
			// reset company
			$this->init_dirs();
		}
	...
	//
	// This filter adds ability to share objects per companies
	//
	function outputfilter_sharing($content, &$compiler)
	{
		$sharing = Registry::get('sharing');
		$content_expr = '/<div id="main_column.*?>.*?<!--main_column.*?--><\/div>/is';
			...
			if (!empty($sharing['tpl_tabs'])) {
				foreach ($sharing['tpl_tabs'] as $object => $data) {
					...
					if (preg_match($tab_expr, $central_content, $matches)) {
						if (!empty($matches[2])) {
							// Add a new tab
							$tab_content = $matches[1] . $matches[2] . '<li id="tab_share_object' . $data['params']['object_id'] . '" class="cm-js cm-ajax"><a href="' . fn_url('companies.get_object_share?object=' . $data['params']['object'] . '&object_id=' . $data['params']['object_id']) . '">' . fn_get_lang_var('share') . '</a></li>' . $matches[3];
