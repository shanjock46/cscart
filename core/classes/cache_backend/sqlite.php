	...
	static function clear($changed_tables)
	{
		...
		if (PRODUCT_TYPE == 'ULTIMATE') {
			foreach (fn_get_all_companies_ids() as $company) {
				$ch = self::_db_fetch("SELECT data FROM cache WHERE name = 'cache_handlers' AND company_id = " . $company);
				$_cache_handlers = !empty($ch['data']) ? @unserialize($ch['data']) : array();
				foreach ($changed_tables as $table => $flag) {
					if (!empty($_cache_handlers[$table])) {
						$tags = array_merge($tags, array_keys($_cache_handlers[$table]));
					}
				}
			}
		}
