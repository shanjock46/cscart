...
class Bm_SchemesManager {
	 ...
	 * @param string $location Dispatch on location to filter for
		...
		foreach($blocks as $block_key => $block) {
			if (!empty($block['type'])) {
				...
				if (!empty($scheme[$type]['hide_on_locations'])) {
					if (array_search($location, $scheme[$type]['hide_on_locations']) !== false) {
						unset($blocks[$block_key]);
					}
				} elseif (!empty($block['type']) && !empty($scheme[$type]['single_for_location'])) {
					$locations = Bm_Block::instance()->where_is_block($type);
