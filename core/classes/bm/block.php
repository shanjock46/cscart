...
class Bm_Block extends CompanySingleton {
	...
	/**
	 * Returns list of locations where block is exists
	 * 
	 * Dynamic object for example, product width id 123:
	 * array(
	 ...
	 * @param string $block_type Block type from scheme
	 * @param array $dynamic_object Dynamic object data 
	 * @return array List of location dispatches
	 */
	public function where_is_block($block_type, $dynamic_object = array())
	{
		$locations = array();
		
		$compay_condition = '';
		if (PRODUCT_TYPE == 'ULTIMATE') {
			$compay_condition = db_quote('AND ?:bm_locations.company_id = ?i', $this->_company_id);
		}

		$all_locations = db_get_array (
			"SELECT dispatch, status, snapping_id FROM ?:bm_locations "
				. "INNER JOIN ?:bm_containers ON ?:bm_containers.location_id = ?:bm_locations.location_id "
				. "INNER JOIN ?:bm_grids ON ?:bm_containers.container_id = ?:bm_grids.container_id "
				. "INNER JOIN ?:bm_snapping ON ?:bm_grids.grid_id = ?:bm_snapping.grid_id "
				. "INNER JOIN ?:bm_blocks ON ?:bm_blocks.block_id = ?:bm_snapping.block_id "
			. "WHERE ?:bm_blocks.type = ?s ?p"
			. "GROUP BY dispatch", 
			$block_type, $compay_condition
		);
		...
		foreach ($all_locations as $location) {
			if (!empty($dynamic_object_statuses[$location['snapping_id']])) {
				$location['status'] = ($location['status'] == 'A') ? 'D' : 'A';
			}
			
			$locations[$location['dispatch']] = $location['status'];
		}
