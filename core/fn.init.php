...
function fn_init_sess_name()
{
	...
	if (PRODUCT_TYPE == 'ULTIMATE' && AREA != 'A') {
		// Ultimate edition: Init session after selected company is initialized
		$sess_postfix .= '_' . (!empty($_REQUEST['s_company']) ? $_REQUEST['s_company'] : 0);
	}
...
function fn_init_templater()
{
	...
	$view->register_outputfilter(array(&$view, 'outputfilter_sharing'));
...
function fn_init_skin($params)
{
	...
	if (defined('SKINS_PANEL')) {
		...
		if (PRODUCT_TYPE == 'ULTIMATE') {
			...
			Registry::set('demo_skin.storefronts', $storefronts);
		}
