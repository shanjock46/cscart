...
function fn_get_recurring_subscriptions($params, $additional_data = array(), $items_per_page = 0)
{
	...
	if (PRODUCT_TYPE == 'ULTIMATE') {
		if (defined('COMPANY_ID')) {
			...
			$condition = db_quote(" AND ?:recurring_plans.company_id = ?i", COMPANY_ID);
		}
	}
...
function fn_get_period_date($timestamp, $duration, $type = 'month')
{
	...
	if ($type == 'month') {
		$new_date = gmmktime($date['hours'], $date['minutes'], $date['seconds'], $date['mon'] + $duration, $date['mday'], $date['year']);
	} elseif ($type == 'day') {
		$new_date = gmmktime($date['hours'], $date['minutes'], $date['seconds'], $date['mon'], $date['mday'] + $duration, $date['year']);
	} elseif ($type == 'year') {
		$new_date = gmmktime($date['hours'], $date['minutes'], $date['seconds'], $date['mon'], $date['mday'], $date['year'] + $duration);
	}
...
function fn_change_subscription_dates($subscription_id, $plan_id, $start_date, $duration)
{
	if (!empty($subscription_id)) {
		...
		if ($plan_data['period'] == 'A') {
			$next_date = gmmktime($date['hours'], $date['minutes'], $date['seconds'], 0, $plan_data['pay_day'], $date['year'] + 1);
			$next_duration = 1;
			$period = 'year';
		} elseif ($plan_data['period'] == 'Q') {
			$next_date = gmmktime($date['hours'], $date['minutes'], $date['seconds'], ceil($date['mon'] / 3) * 3 + 1, $plan_data['pay_day'], $date['year']);
			$period = 'month';
			$next_duration = 3;
		} elseif ($plan_data['period'] == 'M') {
			$next_date = gmmktime($date['hours'], $date['minutes'], $date['seconds'], $date['mon'] + 1, $plan_data['pay_day'], $date['year']);
			$period = 'month';
			$next_duration = 1;
		} elseif ($plan_data['period'] == 'W') {
			$day = $date['wday'] >= $plan_data['pay_day'] ? 7 - $date['wday'] + $plan_data['pay_day'] : $plan_data['pay_day'] - $date['wday'];
			$next_date = gmmktime($date['hours'], $date['minutes'], $date['seconds'], $date['mon'], $date['mday'] + $day, $date['year']);
			$period = 'day';
			$next_duration = 7;
		} elseif ($plan_data['period'] == 'P') {
			$next_date = gmmktime($date['hours'], $date['minutes'], $date['seconds'], $date['mon'], $date['mday'] + $plan_data['by_period'], $date['year']);
			$period = 'day';
...
function fn_get_recurring_events()
{
	...
	$date = getdate();
	$end_time = gmmktime(23, 59, 59, $date['mon'], $date['mday'], $date['year']);
	...
	if (PRODUCT_TYPE == 'ULTIMATE') {
		if (defined('COMPANY_ID')) {
			$completed_subscriptions_join .= db_quote(" INNER JOIN ?:recurring_plans ON ?:recurring_plans.plan_id = ?:recurring_subscriptions.plan_id AND ?:recurring_plans.company_id = ?i", COMPANY_ID);
		}
	}
...
function fn_ult_get_recurring_event_condition($delete_condition, $join)
{
	if (PRODUCT_TYPE == 'ULTIMATE') {
		if (defined('COMPANY_ID')) {
			...
			$join .= db_quote(" INNER JOIN ?:recurring_plans ON ?:recurring_plans.plan_id = subs.plan_id AND ?:recurring_plans.company_id = ?i", COMPANY_ID);
		}
	}
