...
function fn_update_recurring_plan($data, $plan_id, $lang_code = DESCR_SL)
{
	...
	if (!empty($plan_id)) {
		...
		if (PRODUCT_TYPE == 'ULTIMATE') {
			if (defined('COMPANY_ID')) {
				...
				if (COMPANY_ID != $company_id) {
					...
					fn_set_notification('W', fn_get_lang_var('warning'), fn_get_lang_var('rb_please_select_store_to_update'));
				...
				fn_set_notification('W', fn_get_lang_var('warning'), fn_get_lang_var('rb_please_select_store'));
		...
		if (PRODUCT_TYPE == 'ULTIMATE') {
			$data['company_id'] = defined('COMPANY_ID') ? COMPANY_ID : 1;
		}
