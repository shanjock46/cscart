	...
	if ($action == 'step_1') {
		...
		if (!empty($_REQUEST['store_data']['path']) && is_dir($_REQUEST['store_data']['path'])) {
			...
			if ($config !== false) {
				...
				fn_set_notification('E', fn_get_lang_var('error'), fn_get_lang_var('es_this_is_not_cscart_path'));
			}
		} else {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	if ($action == 'step_2') {
		...
		if (empty($store_data['path']) || !is_dir($store_data['path'])) {
			fn_set_notification('E', fn_get_lang_var('error'), fn_get_lang_var('es_cscart_path_not_found'));
			$step = 2;
		}
