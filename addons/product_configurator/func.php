...
function fn_product_configurator_get_products_post(&$products)
{
	foreach ($products as $pr_id => $product) {
		if ($product['product_type'] == 'C') {
			$products[$pr_id]['price'] += fn_pconf_get_default_configuration_price($product);
		}
	}
...
function fn_product_configurator_prepare_product_quick_view($params)
{
	...
	$conf_price = fn_pconf_get_default_configuration_price($product);
	$product['price'] += $conf_price;
	if (!empty($product['discount'])) {
		$product['original_price'] += $conf_price;
	}
 ...
 */
function fn_pconf_get_default_configuration_price($product)
{
	$price = 0;
	...
	if ($product['product_type'] == 'C') {
		...
		if (!empty($conf_product_groups)) {
			$price += fn_pconf_get_configuration_price($conf_product_groups);
		}
	}
