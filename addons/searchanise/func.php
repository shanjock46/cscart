...
fn_define('SE_PRODUCTS_PER_PASS', 200);
fn_define('SE_USE_RELEVANCE_AS_DEFAULT_SORTING', 'Y');// Y, N
...
function fn_se_check_product_filter_block()
{
	...
	$dynamic_object_scheme = Bm_SchemesManager::get_dynamic_object($_REQUEST['dispatch'], AREA);

	if (!empty($dynamic_object_scheme)) {
		$dynamic_object = array(
			'object_type' => $dynamic_object_scheme['object_type'],
			'object_id' => !empty($_REQUEST[$dynamic_object_scheme['key']])? $_REQUEST[$dynamic_object_scheme['key']] : 0,
		);
	}

	$locations = Bm_Block::instance()->where_is_block('product_filters', $dynamic_object);

	if (!empty($locations[$_REQUEST['dispatch']]) && $locations[$_REQUEST['dispatch']] == 'A') {
		$status = true;
	} else {
		$status = false;
	}
...
function fn_searchanise_get_filters_products_count($params)
{
	...
	if (is_null($received_facets) || $get_custom) {
		...
		Registry::set('searchanise.received_facets', $received_facets);
	}
	...
	if (empty($is_select_found) && !empty($params['features_hash'])) {
