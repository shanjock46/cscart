	...
	if (!empty($user_id)) {
		...
		if (AREA == 'A') {
			...
			if (PRODUCT_TYPE == 'ULTIMATE') {
				...
				if (Registry::get('settings.Stores.share_users') == 'Y' && defined('COMPANY_ID')) {
					$orders_ids = db_get_fields("SELECT order_id FROM ?:orders WHERE user_id = ?i AND company_id = ?i", $user_id, COMPANY_ID);
					if (empty($orders_ids)) {
						return array(CONTROLLER_STATUS_NO_PAGE);
					}
				}
