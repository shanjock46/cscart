...
{
	// Get needed version
	$version = fn_twigmo_get_next_version_info();
...
{
	// Restore langvars - for all languages exept EN and RU
	$languages = fn_get_languages();
	$exept_langs = array('EN', 'RU');
	foreach ($languages as $language) {
		$backup_file = $upgrade_dirs['backup_settings'] . 'lang_' . $language['lang_code'] . '.bak';
		if (!in_array($language['lang_code'], $exept_langs) and file_exists($backup_file)) {
	...
	// Update version
	$new_version = fn_twigmo_get_next_version_info();
	$new_version = $new_version['next_version'];
	fn_update_twigmo_options(array('version_forced' => $new_version));
	// Restore companys settings
	foreach ($upgrade_dirs['backup_company_settings'] as $company_id => $dir) {
		$company_settings = unserialize(fn_get_contents($dir . 'settings.bak'));
		$company_settings['version_forced'] = $new_version;
