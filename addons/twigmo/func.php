...
function fn_twigmo_before_dispatch()
{
	...
	if ((isset($_SESSION['use_mobile_frontend']) && !empty($_SESSION['use_mobile_frontend']) && $_SESSION['use_mobile_frontend'] == 'N') || (isset($tw_settings['use_mobile_frontend']) && $tw_settings['use_mobile_frontend'] == 'never')) {
		if (!empty($_SESSION['use_mobile_frontend'])) {
	...
	if ($use_mobile_template && !empty($_SESSION['cache_mobile_frontend'])) {
		$use_mobile_template = ($_SESSION['cache_mobile_frontend'] == "Y") ? true : false;
	}

	if ($use_mobile_template && empty($_SESSION['cache_mobile_frontend'])) {
		if (($force_frontend_view != 'mobile' && empty($_SESSION['use_mobile_frontend'])) || $force_frontend_view == 'auto'){
            if (!isset($_SESSION['twg_user_agent']) || empty($_SESSION['twg_user_agent']) || (!isset($_SESSION['twg_ua']) || $_SESSION['twg_ua'] != $_SERVER['HTTP_USER_AGENT'])) {
			$user_agent = fn_http_request('GET', 'http://twigmo.com/svc2/check_ua.php?ua=' . urlencode($current_agent) . '&access_id=' . Registry::get('addons.twigmo.access_id'));
			$user_agent = $user_agent[1];
			$user_agent = in_array($user_agent,$agent_types) ? $user_agent : "unknown";
	...
	foreach ($block_grids as $block_grid) {
		foreach ($block_grid as $block) {
			...
			if ($block['type'] == 'html_block') {
				...
				// Filter pages - only texts and links posible
				if ($block['type'] == 'pages') {
					foreach ($items as $item_id => $item) {
						if (!in_array($item['page_type'], array('T', 'L', 'F'))) {
							unset($items[$item_id]);
						}
					}
				}
