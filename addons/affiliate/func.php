...
function fn_get_aff_banners($types = array('T', 'G'), $links_to = array('C', 'P', 'U'), $is_avail = false, $lang_code = CART_LANGUAGE)
{
	...
	if (PRODUCT_TYPE == 'ULTIMATE') {
		if (defined('COMPANY_ID')) {
			$conditions[] = db_quote("?:aff_banners.company_id = ?i", COMPANY_ID);
		}
	}
...
function fn_get_date_of_payment_period($cnt_last_periods = 1)
{
	...
	switch (Registry::get('addons.affiliate.payment_period')) {
		case '1w':
			...
			$start_date = gmmktime(0, 0, 0, $cur_date['mon'], $_day, $cur_date['year']) - 7 * 24 * 3600 * $cnt_period;
			break;
		case '2w':
			$checkpoint_1 = 1;
			$checkpoint_2 = 16;
			...
			if (floor($cnt_period/2) != $cnt_period/2) {
				if ($cur_date['mday']<$checkpoint_2) {
					$start_date = gmmktime(0, 0, 0, $cur_date['mon'] - floor($cnt_period / 2) - 1, $checkpoint_2, $cur_date['year']);
					$checkpoint_return = true;
				} else {
					$start_date = gmmktime(0, 0, 0, $cur_date['mon'] - floor($cnt_period / 2), $checkpoint_1, $cur_date['year']);
					$checkpoint_return = false;
				}
			} else {
				$start_date = gmmktime(0, 0, 0, $cur_date['mon'] - floor($cnt_period / 2), $_day, $cur_date['year']);
			}
			break;
		case '1m':
			$_day = 1;
			$start_date = gmmktime(0, 0, 0, $cur_date['mon'], $_day, $cur_date['year']);
...
function fn_get_groups_list($status = 'A', $lang_code = CART_LANGUAGE)
{
	...
	if (PRODUCT_TYPE == 'ULTIMATE') {
		if (defined('COMPANY_ID')) {
			$condition .= db_quote(" AND ?:aff_groups.company_id = ?i", COMPANY_ID);
		}
	}
