...
function fn_correct_gift_certificate(&$gift_cert_data)
{
	...
	if (PRODUCT_TYPE == 'ULTIMATE' && !empty($gift_cert_data['gift_cert_code'])) {
...
function fn_gift_certificate_notification(&$gift_cert_data, $force_notification = array())
{
	...
	if ($notify_user == true && $gift_cert_data['email'] && $gift_cert_data['send_via'] == 'E') {
		...
		if (PRODUCT_TYPE == 'ULTIMATE' && !empty($gift_cert_data['gift_cert_code'])) {
			$company = db_get_row('SELECT ?:companies.storefront, ?:companies.company FROM ?:companies JOIN ?:gift_certificates ON ?:gift_certificates.gift_cert_id = ?i AND ?:companies.company_id = ?:gift_certificates.company_id', $gift_cert_data['gift_cert_id']);
			$gift_cert_data['http_location'] = !empty($company['storefront']) ? 'http://' . $company['storefront'] : (!empty($gift_cert_data['http_location']) ? $gift_cert_data['http_location'] : '');
			$gift_cert_data['company_name'] = !empty($company['company']) ? $company['company'] : (!empty($gift_cert_data['company_name']) ? $gift_cert_data['company_name'] : '');
		}
