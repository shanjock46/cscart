...
{
	if (isset($object_data['seo_name']) && !empty($object_id)) {
		$_object_name = '';
...
function fn_seo_get_products_post($products)
{
	if (AREA == 'C' && !empty($products)) {
		...
		if (Registry::get('addons.seo.seo_product_type') == 'product_category') {
			$id_paths = db_get_array("SELECT pc.product_id, c.id_path, pc.link_type FROM ?:products_categories as pc LEFT JOIN ?:categories as c ON pc.category_id = c.category_id WHERE pc.product_id IN (?n) ?p", $product_ids, fn_get_seo_company_condition('c.company_id'));
...
function fn_seo_url_post(&$url, $area = AREA, $delimeter, $original_url = '', $prefix = 'rel', $company_id_in_url = false, $lang_code = CART_LANGUAGE)
{
	...
	if (!empty($parced_url['path']) && empty($parced_url['query']) && $parced_url['path'] == $index_script) {
		$url = $current_path . ((Registry::get('addons.seo.seo_language') == 'Y') ? fn_strtolower($lang_code) . '/' : '');
		return $url;
	}
	...
	$link_parts = array(
		...
		'lang_code' => (Registry::get('addons.seo.seo_language') == 'Y') ? fn_strtolower($lang_code) . '/' : '',
		'parent_items_names' => '',
		'name' => '',
		'page' => '',
		'extension' => '',
