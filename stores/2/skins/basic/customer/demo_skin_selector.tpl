...
<ul class="demo-site-panel clearfix">
	...
	<li class="dp-area">
		...
		<select name="area" onchange="$.redirect(this.value);">
			...
			{foreach from=$demo_skin.storefronts item=company}
				<option value="http://{$company.storefront}" {if $area == "C" && "COMPANY_ID"|defined && $smarty.const.COMPANY_ID == $company.company_id}selected="selected"{/if}>{$company.company} storefront</option>
			{/foreach}
		</select>
