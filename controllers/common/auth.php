		...
		if (!empty($_REQUEST['user_email'])) {
			...
			if (!empty($u_data['email'])) {
				...
				if (PRODUCT_TYPE == 'ULTIMATE') {
					$company_name = fn_get_company_name($u_data['company_id']);
					$from_email['email'] = CSettings::instance()->get_value('company_users_department', 'Company', $u_data['company_id']);
					$from_email['name'] = $company_name;
					$view_mail->assign('company_name', $company_name);
				}
