...
if ($mode == 'update') {
	...
	if (PRODUCT_TYPE == 'ULTIMATE' && !defined('COMPANY_ID')) {
		$view->assign('picker_selected_companies', fn_ult_get_controller_shared_companies($_REQUEST['feature_id']));
	}
	...
	if (!empty($_REQUEST['feature_id'])) {
		...
		if (PRODUCT_TYPE == 'ULTIMATE') {
			if (!fn_check_company_id('product_features', 'feature_id', $_REQUEST['feature_id'])) {
	...
	list($variants, $total) = fn_get_product_feature_variants($_REQUEST['feature_id'], 0, $_REQUEST['feature_type'], true, DESCR_SL, $items_per_page);
