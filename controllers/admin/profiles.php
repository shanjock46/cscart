	...
	if ($expire) {
		...
		if (!empty($recepients)) {
			foreach ($recepients as $v) {
				...
				if (PRODUCT_TYPE == 'ULTIMATE') {
					$_user_data['company_name'] = fn_get_company_name($_user_data['company_id']);
					$from_email['email'] = CSettings::instance()->get_value('company_users_department', 'Company', $_user_data['company_id']);
					$from_email['name'] = $_user_data['company_name'];
				}
