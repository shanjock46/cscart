...
if (PRODUCT_TYPE == 'ULTIMATE' && $mode != 'edit' && $mode != 'new') {
	if (
		(defined('COMPANY_ID') && !empty($cart['order_company_id']) && COMPANY_ID != $cart['order_company_id'])
		|| (!defined('COMPANY_ID') && !empty($cart['order_company_id']))
	) {
		if (defined('COMPANY_ID')) {
			fn_set_notification('W', fn_get_lang_var('warning'), fn_get_lang_var('orders_not_allow_to_change_company'));
		}
		
		$is_exists = db_get_field("SELECT COUNT(*) FROM ?:companies WHERE company_id = ?i", $cart['order_company_id']);

		if ($is_exists) {
			return array(CONTROLLER_STATUS_REDIRECT, fn_link_attach(Registry::get('config.current_url'), 's_company=' . $cart['order_company_id']));
		} else {
			return array(CONTROLLER_STATUS_DENIED);
		}

	} elseif (empty($cart['order_company_id']) && defined('COMPANY_ID')) {
		$cart['order_company_id'] = COMPANY_ID;
	}
}
