...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if ($mode == 'update') {
		...
		if ($field_data['field_name'] == 'email' && ($field_data['checkout_show'] != 'Y' || $field_data['profile_show'] != 'Y')) {
			$params['field_id'] = $field_data['matching_id'];
			$matching_field = fn_get_profile_fields('ALL', array(), DESCR_SL, $params);

			$show_notification = false;
			if ($field_data['checkout_show'] != 'Y' && $matching_field['checkout_show'] != 'Y') {
				$field_data['checkout_show'] = 'Y';
				
				$show_notification = true;
			}

			if ($field_data['profile_show'] != 'Y' && $matching_field['profile_show'] != 'Y') {
				$field_data['profile_show'] = 'Y';
				
				$show_notification = true;
			}

			if ($show_notification) {
				fn_set_notification('W', fn_get_lang_var('warning'), fn_get_lang_var('email_field_must_be_selected'));
			}
		}
		...
		if (!empty($_REQUEST['fields_data'])) {
			foreach ($_REQUEST['fields_data'] as $field_id => $data) {
				/* Workaround for "email" field */
					if ($data['field_name'] == 'email' && ($data['checkout_show'] != 'Y' || $data['profile_show'] != 'Y') && $data['section'] == 'B') {

						$show_notification = false;

						if ($data['checkout_show'] != 'Y' && $_REQUEST['fields_data'][$data['matching_id']]['checkout_show'] != 'Y') {
							$data['checkout_show'] = 'Y';
							$show_notification = true;
						}
						
						if ($data['profile_show'] != 'Y' && $_REQUEST['fields_data'][$data['matching_id']]['profile_show'] != 'Y') {
							$data['profile_show'] = 'Y';
							$show_notification = true;
						}
