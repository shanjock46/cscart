...
if ($mode == 'index') {
	...
	$date['today'] = date($date_format, gmmktime(0, 0, 0, $today['mon'], $today['mday'], $today['year']));
	$orders_stats['daily_orders'] = db_get_hash_array(
		...
		'status', gmmktime(0, 0, 0, $today['mon'], $today['mday'], $today['year']), TIME
	);
	$orders_stats['daily_orders']['totals'] = db_get_row(
		...
		$paid_statuses, gmmktime(0, 0, 0, $today['mon'], $today['mday'], $today['year']), TIME
	);
	$orders_stats['daily_orders']['prev_totals'] = db_get_row(
		"SELECT SUM(IF(status IN (?a), total, 0)) as total_paid, SUM(total) as total, COUNT(*) as amount "
		. "FROM ?:orders WHERE timestamp >= ?i AND timestamp <= ?i $condition " . fn_get_company_condition('?:orders.company_id'), 
		$paid_statuses, gmmktime(0, 0, 0, $last_day['mon'], $last_day['mday'], $last_day['year']), gmmktime(0, 0, 0, $last_day['mon'], $last_day['mday'], $last_day['year']) + SECONDS_IN_DAY
	);
	...
	$date['week'] = date($date_format, gmmktime(0, 0, 0, $wstart['mon'], $wstart['mday'], $wstart['year']));
	$orders_stats['weekly_orders'] = db_get_hash_array(
		...
		'status', gmmktime(0, 0, 0, $wstart['mon'], $wstart['mday'], $wstart['year']), TIME
	);
	$orders_stats['weekly_orders']['totals'] = db_get_row(
		"SELECT SUM(IF(status IN (?a), total, 0)) as total_paid, SUM(total) as total, COUNT(*) as amount "
		. "FROM ?:orders WHERE timestamp >= ?i AND timestamp <= ?i $condition " . fn_get_company_condition('?:orders.company_id'), 
		$paid_statuses, gmmktime(0, 0, 0, $wstart['mon'], $wstart['mday'], $wstart['year']), TIME
	);
	$orders_stats['weekly_orders']['prev_totals'] = db_get_row(
		"SELECT SUM(IF(status IN (?a), total, 0)) as total_paid, SUM(total) as total, COUNT(*) as amount "
		. "FROM ?:orders WHERE timestamp >= ?i AND timestamp <= ?i $condition " . fn_get_company_condition('?:orders.company_id'), 
		$paid_statuses, gmmktime(0, 0, 0, $last_wstart['mon'], $last_wstart['mday'], $last_wstart['year']), (gmmktime(0, 0, 0, $today['mon'], $today['mday'], $today['year']) - (6 * SECONDS_IN_DAY))
	);
	...
	// Monthly orders
	$date['month'] = date($date_format, gmmktime(0, 0, 0, $today['mon'], 1, $today['year']));
	$last_month = $today;
	if ($last_month['mon'] == 1) {
		$last_month['mon'] = 12;
		$last_month['year']--;
	...
	$orders_stats['monthly_orders'] = db_get_hash_array(
		...
		. "WHERE timestamp >= ?i AND timestamp <= ?i $condition " . fn_get_company_condition('?:orders.company_id') . " GROUP BY status", 
		'status', gmmktime(0, 0, 0, $today['mon'], 1, $today['year']), TIME
	);
	$orders_stats['monthly_orders']['totals'] = db_get_row(
		"SELECT SUM(IF(status IN (?a), total, 0)) as total_paid, SUM(total) as total, COUNT(*) as amount "
		. "FROM ?:orders WHERE timestamp >= ?i  AND timestamp <= ?i $condition " . fn_get_company_condition('?:orders.company_id'), 
		$paid_statuses, gmmktime(0, 0, 0, $today['mon'], 1, $today['year']), TIME
	);
	$orders_stats['monthly_orders']['prev_totals'] = db_get_row(
		"SELECT SUM(IF(status IN (?a), total, 0)) as total_paid, SUM(total) as total, COUNT(*) as amount "
		. "FROM ?:orders WHERE timestamp >= ?i  AND timestamp <= ?i $condition " . fn_get_company_condition('?:orders.company_id'), 
		$paid_statuses, gmmktime(0, 0, 0, $last_month['mon'], 1, $last_month['year']), gmmktime(0, 0, 0, $last_month['mon'], 1, $last_month['year']) + (gmmktime(0, 0, 0, $today['mon'], $today['mday'], $today['year']) + SECONDS_IN_DAY - gmmktime(0, 0, 0, $today['mon'], 1, $today['year']))
	);
	...
	$orders_stats['year_orders'] = db_get_hash_array(
		"SELECT status, COUNT(*) as amount, SUM(total) as total FROM ?:orders "
		. "WHERE timestamp >= ?i AND timestamp <= ?i $condition " . fn_get_company_condition('?:orders.company_id') . " GROUP BY status", 
		'status', gmmktime(0, 0, 0, 1, 1, $today['year']), TIME
	);
	$orders_stats['year_orders']['totals'] = db_get_row(
		...
		$paid_statuses, gmmktime(0, 0, 0, 1, 1, $today['year']), TIME
	);
