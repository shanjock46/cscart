...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if ($mode == 'delete') {
		...
		if (!empty($_REQUEST['promotion_ids'])) {
			...
			if (PRODUCT_TYPE == 'ULTIMATE') {
				foreach($_REQUEST['promotion_ids'] as $k => $v) {
					if (!fn_check_company_id('promotions', 'promotion_id', $v)) {
						unset($_REQUEST['promotion_ids'][$k]);
					}
				}
			}
...
if ($mode == 'update') {
	...
	if (PRODUCT_TYPE == 'ULTIMATE' && !defined('COMPANY_ID')) {
		$view->assign('picker_selected_companies', fn_ult_get_controller_shared_companies($_REQUEST['promotion_id']));
	}
...
// Add promotion
	...
	if (PRODUCT_TYPE == 'ULTIMATE' && !defined('COMPANY_ID')) {
		$view->assign('picker_selected_companies', fn_ult_get_controller_shared_companies($_REQUEST['promotion_id'], 'promotions', 'update'));
	}
