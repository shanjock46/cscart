		...
		{assign var="name" value=$settings.Company.company_name|truncate:40:"...":true}
			...
			{if $smarty.const.PRODUCT_TYPE == "ULTIMATE"}
				{if "COMPANY_ID"|defined}
					<a href="{$index_script|fn_url}">{$name}</a><a href="http://{$s_companies.$s_company.storefront|escape}" class="view-storefront-icon" target="_blank" title="{$lang.view_storefront}">&nbsp;</a>
				{else}
					<a href="{$index_script|fn_url}">{$name}</a>
				{/if}
			{else}
