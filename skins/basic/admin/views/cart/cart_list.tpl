...
<tr>
	...
	<th width="15%"><a class="cm-ajax{if $search.sort_by == "company_id"} sort-link-{$search.sort_order}{/if}" href="{"`$c_url`&amp;sort_by=company_id&amp;sort_order=`$search.sort_order`"|fn_url}" rev="pagination_contents">{$lang.vendor}</a></th>
	{/if}
...
<tr class="table-row">
	<td class="center">
		...
		<input type="checkbox" name="user_ids[{$customer.company_id}][]" value="{$customer.user_id}" class="checkbox cm-item" /></td>
		

	<td>
		
		<img src="{$images_dir}/plus.gif" width="14" height="9" border="0" alt="{$lang.expand_sublist_of_items}" title="{$lang.expand_sublist_of_items}" id="on_user_{$customer.user_id}_{$customer.company_id}" class="hand cm-combination-carts" onclick="$.ajaxRequest('{"cart.cart_list?user_id=`$customer.user_id`&c_company_id=`$customer.company_id`"|fn_url:'A':'rel':'&'}', {$ldelim}result_ids: 'cart_products_{$customer.user_id}_{$customer.company_id}, wishlist_products_{$customer.user_id}_{$customer.company_id}', caching: true{$rdelim});" />
		<img src="{$images_dir}/minus.gif" width="14" height="9" border="0" alt="{$lang.collapse_sublist_of_items}" title="{$lang.collapse_sublist_of_items}" id="off_user_{$customer.user_id}_{$customer.company_id}" class="hand hidden cm-combination-carts" />
	...
	{if !"COMPANY_ID"|defined}
	<td>
		{$customer.company_id|fn_get_company_name}
	</td>
	{/if}
