				  ...
				  <div class="content grid_11 push_2">
					...
					<?php elseif ($mode == 'requirements'): ?>
						<div class="requirements">
							...
							<div class="row">
								<h5><?php echo tr('php_version') ?></h5>
								<span class="f-gray"><?php echo tr('text_php_version_notice', REQUIRED_PHP_VERSION) ?></span>
								<div class="right-column">
									<span class="status <?php if ($php_status == FAIL_MSG){ ?>off<?php } ?>"><?php echo $php_status; ?></span>
									<span class="value"><?php echo $php_value; ?></span>
								</div>
							</div>
							<div class="row">
					...
					<?php elseif ($mode == 'summary'): ?>
						...
						<?php 
							...
							if ($install_demo) {
								$storefront_url = tr("acme_link", "http://$config[http_host]$config[http_path]/acme/$config[customer_index]", "http://$config[http_host]$config[http_path]/acme/$config[customer_index]");
							} else {
								$storefront_url = "";
							}
							...
							echo tr(
									...
									tr('ult_text_summary_notice', "http://$config[http_host]$config[http_path]/acme/$config[customer_index]", "http://$config[http_host]$config[http_path]/acme/$config[customer_index]", DIR_ROOT . "/acme"),
