...
if ($mode == 'requirements') {
	...
	if ($can_continue == true) {
		...
		if ($can_continue == false) {
			...
			if (defined('CONSOLE')) {
				if ($php_error) {
					fn_echo(tr('php_version') . ': ' . $php_value . '. ' . tr('text_php_version_notice', REQUIRED_PHP_VERSION) . "\n\n");
				}
				if ($mysql_error) {
...
if ($mode == 'settings') {
	...
	if (!is_writable(DIR_ROOT . '/stores')) {
		fn_error_msg(tr('text_directory_not_writable', DIR_ROOT . '/stores'), $can_continue, $error_msg);
	}
	if (!is_writable(DIR_ROOT . '/acme/.htaccess')) {
		fn_error_msg(tr('text_file_not_writable', DIR_ROOT . '/acme/.htaccess'), $can_continue, $error_msg);
	}
...
EOT;
	...
	if (!empty($_REQUEST['additional_languages']) && is_array($_REQUEST['additional_languages'])) {
		...
		if (PRODUCT_TYPE == 'ULTIMATE') {
			db_query(
				"REPLACE INTO cscart_ult_objects_sharing 
					(`share_company_id`, `share_object_id`, `share_object_type`) 
				SELECT cc.company_id, d.lang_code, 'languages' 
				FROM cscart_languages d INNER JOIN cscart_companies cc;
			");
		}
...
function fn_install_skin($skin_name)
{
	...
	if (PRODUCT_TYPE == 'ULTIMATE') {
		...
		foreach(fn_get_all_companies_ids() as $company) {
			fn_echo(tr('install_store_skin'));

			$destination = DIR_ROOT . '/' . DIR_STORES . $company . '/skins/' . $skin_name;

			fn_install_skin_zones($skin_name, $zones, $destination, $silent, false);

			CSettings::instance()->update_value('skin_name_customer', $skin_name, '', false, $company);
		}

		// Install root mail zone
		fn_install_skin_zones($skin_name, 'mail', DIR_SKINS . $skin_name, $silent);

		// Copy default 404 page for undefined stores.
		fn_mkdir(DIR_SKINS . $skin_name . '/customer');
		fn_copy(DIR_INSTALL_SKINS . '/' . BASE_SKIN . '/customer/404.tpl', DIR_SKINS . $skin_name . '/customer/exception.tpl', false);
